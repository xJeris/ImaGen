...
_MODEL_DIR = config.ARCH_MODEL_DIRS["Pony"]
_LORA_DIR = config.ARCH_LORA_DIRS["Pony"]
...
class PonyGenerator:
    ...
    def load_model(self, model_name=None, progress_callback=None):
        ...
        if not model_name:
            ...
            if not models:
                raise FileNotFoundError("No Pony models found in models/pony/")
            model_name = models[0]
        ...
        if progress_callback:
            progress_callback(f"Loading {self._model_name} (pony)...")
